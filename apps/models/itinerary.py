...
class Itinerary(BaseModel):
    ...
    itinerary_id = models.CharField(max_length=20, primary_key=True, editable=False, verbose_name='行程唯一标识', db_comment='行程唯一标识符,格式为ITI_YYYYMMDD_XXX')
    itinerary_name = models.CharField(max_length=100, verbose_name='行程名称', db_comment='行程名称')
    description = models.TextField(null=True, blank=True, verbose_name='行程描述', db_comment='行程详细描述')
    travel_purpose = models.CharField(max_length=20, choices=TravelPurpose.choices, verbose_name='旅行目的', db_comment='旅行目的,如休闲、商务、蜜月等')
    start_date = models.DateField(verbose_name='行程开始日期', db_comment='行程开始日期')
    end_date = models.DateField(verbose_name='行程结束日期', db_comment='行程结束日期')
    total_days = models.IntegerField(verbose_name='总天数', db_comment='行程总天数,自动计算')
    ...
    budget_flexibility = models.CharField(max_length=10, choices=BudgetFlexibility.choices, null=True, blank=True, verbose_name='预算灵活性', db_comment='预算灵活性,如严格、适中、灵活')
    current_status = models.CharField(max_length=20, choices=CurrentStatus.choices, verbose_name='当前状态', db_comment='行程当前状态,如草稿、待审核、已确认等')
    review_deadline = models.DateTimeField(null=True, blank=True, verbose_name='审核截止时间', db_comment='审核截止时间')
    expiration_date = models.DateField(null=True, blank=True, verbose_name='行程有效期截止日期', db_comment='行程有效期截止日期')
    confirmed_by = models.CharField(max_length=50, null=True, blank=True, verbose_name='确认人（运营人员）', db_comment='确认行程的运营人员')
    confirmed_at = models.DateTimeField(null=True, blank=True, verbose_name='确认时间', db_comment='行程确认时间')
    is_template = models.BooleanField(default=False, verbose_name='是否为模板', db_comment='标识是否为行程模板')
    template_name = models.CharField(max_length=100, null=True, blank=True, verbose_name='模板名称', db_comment='模板名称')
    template_category = models.CharField(max_length=20, choices=TemplateCategory.choices, null=True, blank=True, verbose_name='模板分类', db_comment='模板分类,如蜜月、家庭、冒险等')
    usage_count = models.IntegerField(default=0, validators=[MinValueValidator(0)], verbose_name='模板使用次数', db_comment='模板被使用的次数')
    last_used = models.DateTimeField(null=True, blank=True, verbose_name='模板最后使用时间', db_comment='模板最后使用时间')
    created_by = models.CharField(max_length=50, verbose_name='创建人', db_comment='行程创建人用户名')
    updated_by = models.CharField(max_length=50, null=True, blank=True, verbose_name='更新人', db_comment='行程更新人用户名')
    version = models.IntegerField(default=1, verbose_name='版本号', db_comment='数据版本号,用于版本控制')
    ...
    class Meta:
        ...
        db_table_comment = '旅游行程规划主表,存储完整的旅游行程信息,包括行程基本信息、预算、状态、模板等核心数据'
